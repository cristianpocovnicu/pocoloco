...
export default function SocialAuthButtons({ next }: { next?: string }) {
  ...
  const handleGoogle = async () => {
    ...
    // dacă merge, browserul pleacă spre Google și nu mai ajungem aici
    if (oauthError) {
      setError(
  ...
  return (
    <div className="flex flex-col gap-2.5 mb-5">
      ...
      {/* Apple Sign In nu e configurat încă — buton inactiv, nu unul care nu face nimic */}
      <button
        disabled
        title="Disponibil în curând"
        className="w-full py-3.5 rounded-full bg-black/40 flex items-center justify-center gap-2.5 font-outfit text-[14px] font-medium text-white cursor-not-allowed"
      >
        🍎 Apple — în curând
      </button>
    </div>
