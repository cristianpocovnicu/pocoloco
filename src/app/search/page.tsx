    ...
    <main className="pb-nav">
      <div className="content-wrap pt-4">
        <div className="flex items-center gap-2 mb-3">
          <div className="flex-1 bg-white border border-[rgba(0,0,0,0.08)] rounded-full px-4 py-2.5 flex items-center gap-2 shadow-sm">
        ...
        <div className="flex bg-white border border-[rgba(0,0,0,0.08)] rounded-full p-0.5 mb-3 shadow-sm">
          <button onClick={() => setView('list')} className={cn('flex-1 flex items-center justify-center gap-1.5 py-2 rounded-full text-[13px] font-outfit font-medium transition-all', view === 'list' ? 'bg-[#F8F7F5] text-[#0F0F0F] shadow-sm' : 'text-[#9B9B9B]')}>
            <LayoutList size={14} /> Listă
          </button>
          <button onClick={() => setView('map')} className={cn('flex-1 flex items-center justify-center gap-1.5 py-2 rounded-full text-[13px] font-outfit font-medium transition-all', view === 'map' ? 'bg-[#F8F7F5] text-[#0F0F0F] shadow-sm' : 'text-[#9B9B9B]')}>
            <Map size={14} /> Hartă
          </button>
        </div>
