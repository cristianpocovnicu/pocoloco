...
| Notificare în clopoțel la fiecare locație nouă în așteptare | badge-ul din navigație (10 august 2026) rezolvă deja „de unde știu că am de moderat". O notificare ar cere: extinderea a **două** constrângeri `check` din migrarea 6 (`type` și `entity_type`), un trigger care multiplică rândul pentru fiecare admin, și un tip nou în maparea din `lib/notifications.ts` — plus o problemă de gen: clopoțelul e un flux personal („X ți-a apreciat experiența"), iar o coadă de lucru al echipei n-are aceeași natură. De făcut dacă ajung mai mulți moderatori decât o pagină de admin deschisă zilnic |
